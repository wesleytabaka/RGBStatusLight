...
// TRANSITIONMODE, EFFECT, NEW_R, NEW_G, NEW_B | CYCLE
// TRANSITIONMODE: NOW, FADE:DURATION
// EFFECT: SOLID, BLINK:RATE, PULSE:RATE
// CYCLE: Runs through RGB colors.
...
void loop() {
  ...
  if(Serial.available()){    
    ...
    if(input.length() > 0){
      ...
      serialOutput += b;
//  
//      int sum = r.toInt() + g.toInt() + b.toInt();
